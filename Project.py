...
def main():
    ...
    pygame.display.set_caption("Space Shooter: Step 2")
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            #when pressing space bar create a bullets
        ...
        for b in bullets: 
            pygame.draw.rect(screen, WHITE, b) #draw bullets
        
        pygame.display.flip()
        clock.tick(60)

# basic bullet movements.
def update_bullets(bullets):
    """Function 1: Handles bullet movement and removal"""
    for b in bullets[:]:
        b.y -= 10
        if b.bottom < 0:
            bullets.remove(b)
